...
- [x] Switching language in Journey shouldn't jump back to the Home page. *(explicitly deferred by user — "save this fix for later")* **Moot as of a later redesign, confirmed live 2026-08-29**: the language switcher (`#hm-lang-select`) now lives exclusively on the Home tab — `engine/home.js`'s own comment documents the deliberate move ("a course choice... belongs where the learner lands first," not "buried in My Journey"). Verified live: navigating to Journey, the switcher element exists in the DOM (Home's markup stays mounted) but has zero size and no `offsetParent` — not visible or reachable from Journey at all. Since there's no way to switch language from Journey any more, the original interaction this item reported can't happen.
...
- [x] After successfully solving a Spanish exercise, the English translation should always be shown (e.g. "Estudio español porque me gusta" → check → correct → show translation). **Plumbing added 2026-08-18**: plumbing added (`stepState.translation`, shown via `showTranslation()` on solve/reveal) for fill-blank and dictation steps, reading `step.english`/`step.translation` from content. **Content populated 2026-09-21 for A1 & A2**: Added `english` optional property to `fillBlank` and `dictation` schemas in `content/es-es` and `content/es-latam`. Extracted and backfilled natural English translations across all 1,109 exercises (381 A1 fill-blank, 563 A2 fill-blank, 165 A2 dictation) across 332 files in both courses; validated with `validate-content.py` (0 failures).
...
- [x] Grammar screens: audio playback is inconsistently placed. E.g. in a conjugation table you can listen to the pronouns (yo, tú, él) but not the conjugated forms themselves (trabajo, trabajas, trabaja). Sometimes only the English is voiced, not the Spanish. **Fixed 2026-08-18 and fully swept 2026-09-21**: the shared `table()` renderer (engine/lessons.js) adds a listen button on column 2 whenever column 1 is a bare subject pronoun. In addition, the 2026-09-21 audit identified the root cause of "only English is voiced": 88 grammar tables across `es-latam` and `es-es` had been authored with inverted columns (`[English, Spanish]`), causing `.lsn-table` CSS to italicize English and attach Spanish TTS to the English gloss. All 88 tables were normalized to standard `[Spanish, English]` order, and a runtime language detector was added to `stepRenderers.table()` in `engine/lessons.js` to ensure the audio button is always bound to the target language, never the English gloss.
...
- [x] Recurring multiple-choice bug: several answer options are all grammatically valid, but only one is accepted. **Fixed 2026-08-18 and exhaustive sweep completed 2026-09-21**: initially resolved for A1 U9. On 2026-09-21, audited the entire corpus across `es-latam` and `es-es`, identifying 48 ambiguous questions where multiple options were grammatically valid (e.g., `Tengo dos hermanos` vs `Tienen dos hermanos`, `Quiero dos cafés` vs `Quieren dos cafés`, `Duermo` vs `Duermes` vs `Duerme`) under a generic `Which is correct?` prompt. Disambiguated all 48 questions by adding specific English target prompts (e.g., `Which translates "I have two brothers"?`, `Which translates "I sleep at eleven"?`).
...
- ~~Consider a "Show English translation" toggle button for B1 informative texts that reveals a full English version.~~ **Discarded 2026-09-17** — content-blocked (no English text exists for any of the ~175 B1 World paragraphs, making this a translation project, not a UI one) and not worth the investment. Item removed from backlog.
- [x] **B1 LatAm track — fitness-for-purpose audit, 2026-08-27** (user: "I'm not sure that it's fit for purpose as it is"). **Decision made and generation approach fixed 2026-08-27**: rather than simplify/translate/split the existing content, the direction was to rebuild it — reading-first, exercise-light, escalating toward B2, deep enough that finishing a lesson supports "a five to fifteen minute conversation" about the topic. The new approach is written up in full in `content/es/guides/b1-content-spec.md` §3 (superseding its original Latin America section). **Unit 1 (`unit.b1.latam.01`, pre-Columbian societies) is now fully rebuilt, 2026-08-28** — all five teaching lessons, each with its own dedicated story (previously only lesson 05 had a story at all, shared unit-wide and read superficially by all five lessons):
  ...
  **Unit 30 (`unit.b1.latam.30`, "Democratization") also fully rebuilt, 2026-08-28**:
  - `lesson.b1.democratizacion.01` "Volver a votar" — the 1979-1990 wave of transitions to civilian government (Ecuador, Peru, Bolivia, Argentina, Uruguay, Brazil, Paraguay, Chile), contrasting collapsed vs. negotiated exits, and Paraguay's internal 1989 coup against Stroessner.
  - `lesson.b1.democratizacion.02` "Nuevas instituciones" — Brazil's 1988 "Citizen Constitution", Colombia's 1991 constituent assembly and Constitutional Court, Paraguay's 1992 post-Stroessner constitution, and the region-wide rise of independent electoral tribunals.
  - `lesson.b1.democratizacion.03` "Democracia y justicia" — Brazil's markedly different transitional-justice path: its 1979 self-amnesty, upheld by its own Supreme Court in 2010, the same year the Inter-American Court's Gomes Lund ruling found it incompatible with international law (a contrast Brazil still hasn't resolved).
  - `lesson.b1.democratizacion.04` "Las dificultades de la transición" — Guatemala's failed May 1993 "serranazo" self-coup (contrasted with Uruguay's successful 1973 one from Unit 27), plus institutional and budgetary constraints young democracies inherited from the dictatorships and the debt crisis/neoliberalism.
  - `lesson.b1.democratizacion.05` "Democracias imperfectas" — a closing assessment via Guillermo O'Donnell's 1994 "delegative democracy" concept (a deliberate callback to his "bureaucratic-authoritarian state" term from Unit 24), persistent corruption and weak institutional trust, and late-1990s regional polling.

  Same template throughout. `validate-content.py`: 2836/2836 passing. `audit-lesson.py` clean per-lesson (no word-coverage failures). `curriculum.json` updated. Verified live via Playwright across all five lessons: story-before-grammar order, zero console errors. The unit's pre-existing `consolidation` lesson was left untouched.

  **Unit 31 (`unit.b1.latam.31`, "Indigenous Movements") also fully rebuilt, 2026-08-28**:
  - `lesson.b1.movimientosindigenas.01` "Pueblos indígenas" — the region's 40+ million Indigenous people across hundreds of peoples and languages (Quechua, Aymara, Maya, Guaraní, Mapuche, etc.), and most states' near-total lack of official recognition of this diversity through most of the twentieth century.
  - `lesson.b1.movimientosindigenas.02` "Tierra y derechos" — the 1989 ILO Convention 169's prior-consultation right, Nicaragua's 1987 Atlantic Coast autonomous regions, and Colombia's collective-land "resguardo" system.
  - `lesson.b1.movimientosindigenas.03` "Identidad y reconocimiento" — the constitutional-recognition wave: Colombia 1991 (multiethnic/pluricultural nation), Mexico's 1992 Article 4 reform, Ecuador's 2008 "Buen Vivir", and Bolivia's 2009 "Plurinational State".
  - `lesson.b1.movimientosindigenas.04` "Movilización política" — three landmark mobilizations: Ecuador's CONAIE uprisings (1990, and its role in President Mahuad's 2000 fall), Mexico's January 1, 1994 Zapatista uprising, and Evo Morales becoming Bolivia's first Indigenous president in January 2006.
  - `lesson.b1.movimientosindigenas.05` "Nuevas voces" — late-1990s growth in Indigenous political representation, the 2007 UN Declaration on the Rights of Indigenous Peoples, and the persistent poverty gaps and resource-extraction conflicts alongside these gains.

  Same template throughout. `validate-content.py`: 2846/2846 passing. `audit-lesson.py` clean per-lesson (no word-coverage failures). `curriculum.json` updated. Verified live via Playwright across all five lessons: story-before-grammar order, zero console errors. The unit's pre-existing `consolidation` lesson was left untouched.

  **Unit 32 (`unit.b1.latam.32`, "Regional Integration") also fully rebuilt, 2026-08-28**:
  - `lesson.b1.integracionregional.01` "¿Por qué integrarse?" — Bolívar's nineteenth-century unity dream, the 1960 Latin American Free Trade Association, and the 1990s' post-debt-crisis push toward deeper integration.
  - `lesson.b1.integracionregional.02` "Mercosur" — the March 1991 Treaty of Asunción (Argentina, Brazil, Paraguay, Uruguay), Venezuela's 2012 accession and later suspension, and the bloc's persistent Brazil/Argentina-vs-smaller-partners asymmetry.
  - `lesson.b1.integracionregional.03` "Comunidad Andina" — the 1969 Andean Pact (Cartagena Agreement), Chile's 1976 withdrawal under Pinochet, Venezuela's 2006 exit to join Mercosur, and the bloc's 1996 reorganization with its own parliament and court.
  - `lesson.b1.integracionregional.04` "Comercio y cooperación" — NAFTA's (1994) effects on Mexican export manufacturing and small corn farmers, the 1994 Group of Three (Mexico-Colombia-Venezuela), and the resulting "spaghetti bowl" of overlapping trade agreements.
  - `lesson.b1.integracionregional.05` "Una región unida" — a closing assessment of consolidated blocs and lasting institutions against still-modest intra-regional trade, and a forward look to 2008's UNASUR.

  Same template throughout. `validate-content.py`: 2856/2856 passing. `audit-lesson.py` clean per-lesson (no word-coverage failures). `curriculum.json` updated. Verified live via Playwright across all five lessons: story-before-grammar order, zero console errors. The unit's pre-existing `consolidation` lesson was left untouched.

  **Unit 33 (`unit.b1.latam.33`, "The End of the Cold War") also fully rebuilt, 2026-08-28**:
  - `lesson.b1.finalguerrafria.01` "1989" — the November 9, 1989 fall of the Berlin Wall and the December 1989 Malta Summit where Bush and Gorbachov jointly declared the Cold War over.
  - `lesson.b1.finalguerrafria.02` "Un nuevo mundo" — the Soviet Union's December 25, 1991 dissolution, the "unipolar moment," and Fukuyama's 1989 "end of history" essay.
  - `lesson.b1.finalguerrafria.03` "Nuevas prioridades" — US policy's shift from a single anti-communist axis to trade, counter-narcotics and democracy promotion: Bush's June 1990 Enterprise for the Americas Initiative and the 1989 Andean Initiative.
  - `lesson.b1.finalguerrafria.04` "América Latina después de la Guerra Fría" — Cuba's loss of Soviet subsidies, the 1991 "Special Period," its roughly one-third economic contraction, and the August 1994 balsero migration crisis.
  - `lesson.b1.finalguerrafria.05` "Hacia los años noventa" — a closing synthesis of the unit's three storylines and the transformed international framework the region entered the 1990s within.

  Same template throughout. `validate-content.py`: 2866/2866 passing. `audit-lesson.py` clean per-lesson (no word-coverage failures). `curriculum.json` updated. Verified live via Playwright across all five lessons: story-before-grammar order, zero console errors. The unit's pre-existing `consolidation` lesson was left untouched.

  **Unit 34 (`unit.b1.latam.34`, "Latin America in the 1990s") also fully rebuilt, 2026-08-29**:
  - `lesson.b1.latamnoventa.01` "Los años de Menem" — the 1993 Pacto de Olivos, the 1994 constitutional reform enabling Menem's 1995 reelection, the Yomagate money-laundering scandal and illegal arms sales, and "menemismo" as a personal governing style. Deliberately angled toward Menem's political maneuvering, not the privatizations/Convertibility Plan already covered in Unit 29.
  - `lesson.b1.latamnoventa.02` "Fujimori y Perú" — Alberto Fujimori's surprise 1990 election over Mario Vargas Llosa, the "Fujishock" adjustment program, the April 5, 1992 autogolpe dissolving Congress, and the September 1992 capture of Abimael Guzmán.
  - `lesson.b1.latamnoventa.03` "México y el TLCAN" — the January 1, 1994 Zapatista uprising the same day NAFTA took effect, the March 23, 1994 assassination of Luis Donaldo Colosio, and the December 1994 "crisis del tequila" peso devaluation and US-led bailout. Deliberately angled toward 1994's political shocks rather than re-explaining NAFTA's trade effects, already covered in Unit 32.
  - `lesson.b1.latamnoventa.04` "Democracia y mercado" — a comparative synthesis of the region's "democracia delegativa" pattern (O'Donnell), contrasting Menem's and Fujimori's concentration of power with Fernando Collor de Mello's December 1992 impeachment in Brazil and criticism of Carlos Salinas de Gortari in Mexico.
  - `lesson.b1.latamnoventa.05` "Una década de contrastes" — a closing regional comparison contrasting Chile's stable growth with the crises in Mexico and Peru, and CEPAL's finding that the region ended the decade among the world's most unequal, setting up the following units.

  Same template throughout. `validate-content.py`: 2876/2876 passing. `audit-lesson.py` clean per-lesson (no word-coverage failures). `curriculum.json` updated. Verified live via Playwright across all five lessons: story-before-grammar order, zero console errors. The unit's pre-existing `consolidation` lesson was left untouched.

  **Unit 35 (`unit.b1.latam.35`, "The Legacy of the 20th Century") also fully rebuilt, 2026-08-29**:
  - `lesson.b1.legadosigloveinte.01` "Revolución y reforma" — Bolivia's April 1952 National Revolution (universal suffrage, mine nationalization, 1953 agrarian reform) contrasted with Costa Rica's 1948 civil war and its 1949 Constitution abolishing the army.
  - `lesson.b1.legadosigloveinte.02` "Dictadura y democracia" — Brazil's 1988 "Constitución Ciudadana" and Colombia's 1991 Constitution (born of the 1990 "Séptima Papeleta" student movement, creating the Constitutional Court and acción de tutela) as constitutional reform used to expand rather than concentrate power, contrasted with Unit 34's Menem/Fujimori cases.
  - `lesson.b1.legadosigloveinte.03` "Desigualdad y desarrollo" — Mexico's 1997 Progresa conditional cash transfer program (designed by Santiago Levy, rigorously evaluated, later renamed Oportunidades) as a policy response to inequality that outlasted decades of shifting development models.
  - `lesson.b1.legadosigloveinte.04` "Identidad y memoria" — Argentina's official 1984 CONADEP "Nunca Más" report and Brazil's clandestine "Brasil: Nunca Mais" project (Cardinal Paulo Evaristo Arns and pastor Jaime Wright), two independent processes arriving at the same formula.
  - `lesson.b1.legadosigloveinte.05` "¿Qué dejó el siglo XX?" — a closing synthesis of the unit's four themes as the region entered the new millennium.

  Same template throughout. `validate-content.py`: 2886/2886 passing. `audit-lesson.py` clean per-lesson (no word-coverage failures). `curriculum.json` updated. Verified live via Playwright across all five lessons: story-before-grammar order, zero console errors. The unit's pre-existing `consolidation` lesson was left untouched.

  **Unit 36 (`unit.b1.latam.36`, "Latin America Toward 2000") also fully rebuilt, 2026-08-29 — the last Latin America unit, completing the rebuild**:
  - `lesson.b1.americalatinadosmil.01` "Una región transformada" — Hugo Chávez's December 1998 election in Venezuela and the 1999 Constituent Assembly, as the opening sign of a new political chapter for the region.
  - `lesson.b1.americalatinadosmil.02` "Política" — Ecuador's January 2000 crisis: Mahuad's dollarization announcement, the CONAIE-backed takeover of Congress, and Vice President Noboa's decision to keep the dollarization in place.
  - `lesson.b1.americalatinadosmil.03` "Economía y globalización" — remittances as a quietly massive economic force by 2000, especially in El Salvador, where they rivaled traditional exports.
  - `lesson.b1.americalatinadosmil.04` "Sociedad e identidad" — the region's demographic transformation into the developing world's most urbanized region by 2000, its megacities, and informal settlements.
  - `lesson.b1.americalatinadosmil.05` "América Latina al entrar en el nuevo siglo" — the December 31, 1999 handover of the Panama Canal to Panama, closing with a synthesis of the entire 36-unit course arc from pre-Columbian societies onward.

  Same template throughout. `validate-content.py`: 2896/2896 passing. `audit-lesson.py` clean per-lesson (no word-coverage failures). `curriculum.json` updated. Verified live via Playwright across all five lessons: story-before-grammar order, zero console errors. The unit's pre-existing `consolidation` lesson was left untouched.

  **The full 36-unit Latin America track is now rebuilt** to the reading-first, exercise-light model (72 units total across Core+LatAm; this rebuild covers all 36 LatAm units' 180 teaching lessons). Also noted but left alone as out of scope: `scripts/buildStories.js` (regenerates `content/es/stories/manifest.json`) no longer matches the current one-level-deeper `stories/<source>/<level>/*.json` layout and would wipe the manifest to empty if run — the manifest isn't read by any runtime code (only referenced in schema README docs), so this is a dead/stale tool, not a content gap.

  **2026-08-29 correction:** the claim above that the manifest "isn't read by any runtime code" was wrong — `engine/reader.js`'s `ensureStories()`/`renderLibrary()` fetches and renders `stories/manifest.json` directly; it's what drives the entire Library screen. Because it was never regenerated after this rebuild, roughly 250 of the new per-lesson B1 readings (e.g. `story.b1.caudillismo.01`–`.05`) existed on disk but were invisible in the app. Separately, 99 of these same LatAm lessons had their `grammar` summary field silently degraded to a placeholder (just the lesson title) during the rebuild, while `curriculum.json` still carried the real pre-rebuild summaries — the two had drifted apart. Both fixed: the 99 summaries were restored into the actual lesson files, and `content/es/{curriculum,decks,stories/manifest}.json` regenerated to match. `scripts/buildStories.js` has been deleted (along with the `npm run build:stories` entry) — `build-manifest.py` is the one tool that should ever touch these files now. A new CI workflow, `.github/workflows/sync-generated-content.yml`, runs `build-manifest.py` plus the grammar/translation index builders on every push to master and commits the result automatically, so this class of drift shouldn't recur silently again.

  **2026-08-29 follow-up — the other 81:** the 99 restored above were only the LatAm lessons where a real summary survived *somewhere* (in the stale `curriculum.json`). The remaining 81 LatAm lessons had the placeholder in both places — nothing to restore, because nothing had ever been promoted to the lesson's top-level `grammar` field in the first place. But each one *does* have a real, substantive grammar section (`grammar/b1/*.json`, referenced from the lesson's own `sections`) with its own proper title, e.g. `b1-economiacolonial-03.json`'s ("Plantaciones") is "Gerundio: expansión de las plantaciones"; `b1-revolucioncubana-02.json`'s ("Fidel Castro y la revolución") is "Acabar por + infinitivo: cómo terminó un proceso". Verified all 81 have exactly one grammar section with a real, non-placeholder title before touching anything. Promoted each lesson's own grammar-file title into its top-level `grammar` field — the same fix pattern as the 99 above, just pulling from the grammar file instead of the stale curriculum.json, since that was the only place a real summary existed for these. `curriculum.json` regenerated to match. `validate-content.py`: 2896/2896 (es), 869/869 (hu) passing throughout.

  **2026-08-27 initial audit findings (now fully resolved & superseded):**
  - *Scope & Calibration*: Rebuilt with dedicated per-lesson stories and 9-exercise practice blocks across all 36 units.
  - *Translation support*: Full paragraph translation toggle was formally discarded on 2026-09-17 (see line 46) because interactive word-level and multi-word phrase tap-to-translate glosses are already active across all readings via `Reader.makeClickable()`.
  - *Immersion standard*: The content serves as authentic communicative reading practice aligned with the B1+ reading standard.
