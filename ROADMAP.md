...
8. **Name and tag-check the newly authored grammar skills (added 2026-09-24).** The tag audit and naming pass (ACHIEVED.md items 89 and 91) covered content as of 13:32 that day. Since then the es-es CCSE track (Units 7–36) and the Hungarian citizenship track were rewritten, adding 151 es-es skills and 1 hu skill (`synthesis-orszagma`) with no curated name. `scripts/build_grammar_index.py` lists them. Many of the new es-es IDs are whole phrases rather than grammar points (`acoger-el-festival-internacional-de-cine-y-entregar-la-concha-de-oro`), so they need the same treatment: check each exercise's tag against what it tests, then add plain mid-sentence names to `grammar-titles.json`. Also left open by the audit:
