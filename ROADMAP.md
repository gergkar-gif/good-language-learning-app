...
99. **Consolidate near-duplicate and single-use `teaches` skill slugs across courses (added 2026-09-25).** All 34,673 exercises across `es-es`, `es-latam` and `hu` now have validated `category` and `teaches` metadata against `content/<course>/indexes/skill-registry.json` (`ACHIEVED.md` item 98), but ~245–390 tags per course are used only once (e.g. unit-suffixed slugs such as `past-tense-unit24-25` or `possessive-suffixes-43`). Merge near-duplicate and single-use slugs into canonical skills (recording `"aliases": [...]` in `skill-registry.json`), and update every exercise, `diagnostic-test.json`, and `verb-tense-skills.json` reference accordingly.
