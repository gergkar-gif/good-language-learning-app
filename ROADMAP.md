...
98. **Give Hungarian a real grammar skill catalogue (added 2026-09-25).** `content/hu/indexes/grammar-index.json` has only 48 skills, mostly vocabulary and history-unit topics. Core grammar has no skill id at all: cases/suffixes, present and past conjugation, definite vs indefinite, the conditional, verbal prefixes. So when the placement diagnostic's answers were wired into the learner model (ACHIEVED.md item 97), only 1 of the 30 Hungarian questions could be tagged with a real skill, and a Hungarian learner's grammar weaknesses can't be tracked at skill level. The cause is probably the tags on Hungarian lesson exercises (69% tagged) rather than the index builder; check what `scripts/build_grammar_index.py` sees. Then retag the other 29 Hungarian diagnostic questions (their current free-text tags, like `definite-conjugation-2sg`, describe the intent). Two Spanish questions are also unmapped because no matching skill exists: `es-diag-a1-06` (present tense for a polite request, *¿me trae…?*) and `es-diag-b1-05` (por vs para). The tests assert at least 28/30 Spanish tags resolve.
...
- **Exercise Schemas & Teaches Tags**:
  ...
  - **Skill names (`indexes/grammar-titles.json`)**: every `teaches` slug used on a `category: "grammar"` exercise needs a name in `content/<lang>/indexes/grammar-titles.json`. Learners see it mid-sentence ("You made a few mistakes with stem-changing reflexive verbs lately."), so write it lowercase-first in plain CEFR-inventory English, with target-language forms unquoted ("estar + gerund", "ser vs estar"). No colons, dashes or parentheses. A tag should name a grammar point, not a topic or story. Exercises with no grammar in them are named "reading". `scripts/build_grammar_index.py` warns about any unnamed skill (ACHIEVED.md items 89, 91, 93).
