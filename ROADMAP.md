...
Found by pulling the UI strings out of the 12 language-app APKs in `apps for ux-ui inspiration/`. The per-app inventories are in `feature-inventories/` there. Only ideas that fit Parlour and aren't already built are listed. Items 1–2 are gaps confirmed in our code, not just ideas.
1. **Resume a lesson where you left off** (Babbel: "Continue lesson?" → continue or restart). Today nothing about a lesson in progress is saved: `teardownLesson()` resets `currentStepIndex`, so leaving a 25-step lesson halfway on a phone means starting again. Save lesson id, step index, missed steps and the built step list (`buildSteps()` can recycle or shuffle); on reopening, offer Continue / Start over.
2. **Credit vocabulary when a learner tests out** (Memrise, after placement: "We've marked some words as 'known' so you can focus on what's next"). `markLevelComplete()` (`engine/progress.js`), called from the diagnostic and level tests, marks lessons `viaLevelTest` but adds no words to My Dictionary or the deck. So Reader word colours, "% familiar" and "Within reach" treat a learner placed into A2 as knowing no A1 words. Add the skipped levels' taught words as known (or as a weaker "assumed known" status).
