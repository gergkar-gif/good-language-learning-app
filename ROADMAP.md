...
6. **Readable, CEFR-style names for grammar skills (added 2026-09-24).** Practice suggestions now name the skill in a sentence ("You made a few mistakes with … lately", ACHIEVED.md item 88). But `humanizeSkill()` (`engine/recommendationEngine.js`) only reads a curated `indexes/grammar-titles.json`, and only Hungarian has one. Every Spanish skill falls back to a title-cased ID: "Cambio Radical Reflexivos", "Estar Ando", "Gustar Infinitivo". `grammar-index.json`'s generated `titles` (`scripts/build_grammar_index.py`) aren't a fix either. About half are the same fallback, and some are story or lesson titles ("A Full Day", "La crisis de 1929"). Needed: a curated `content/es-es` and `content/es-latam` `grammar-titles.json` with plain, understated names in the vocabulary of a CEFR grammar inventory ("stem-changing reflexive verbs", "estar + gerund", "gustar + infinitive"), written to sit mid-sentence. Also check whether the Hungarian titles read well mid-sentence.
