...
18. **AI-Aware Pre-Generated Reading Narration & Content Annotation Architecture** — **Built & deployed 2026-09-14**:
    ...
    - **Modern Neural TTS Synthesis & Multi-Speaker Casting (`edge-tts`)**: Upgraded synthesis from legacy desktop SAPI to high-definition neural speech models. Casts distinct human voices per character (e.g., Castilian Spanish `es-ES-ElviraNeural` for Narrator, `es-ES-AlvaroNeural` for Carlos, `es-ES-XimenaNeural` for Meg; Hungarian `hu-HU-TamasNeural` for Károly and `hu-HU-NoemiNeural` for Meg; English `en-US-EmmaNeural` for scaffolding).
    - **Mixed-Language Reading Support**: Seamlessly handles stories with target-language dialogue and English scaffolding narration (standard across Hungarian A1/A2 stories). Each paragraph detects its `lang` tag, routing English scaffolding to native English narration and target dialogue to authentic native voices with CEFR-calibrated pacing.
    - **Compact Pre-Generated Audio Assets (`.mp3`)**: Master audio streams are synthesized once during content creation and stored with the reading (`content/<lang>/stories/audio/<story-id>.mp3`), reducing asset sizes by 25x compared to uncompressed audio (~370-420 KB per story) with zero runtime AI API calls or costs.
    - **Synchronized Audio Player & Active Paragraph Highlighter** (`engine/reader.js`): Story player bar with Play/Pause, scrub slider, time counter, and speed selector (`0.8x`, `1.0x`, `1.2x`), defaulting automatically to normal natural human speed (`1.0x`). Automatically tracks audio timestamps to highlight active spoken paragraph (`.story-paragraph.is-narrating`) in real time, with individual paragraph direct-seek buttons (`.story-para-play-btn`).
    - **Batch Offline Content Generation**: `scripts/narrate-story.py` supports single stories, directory traversals, or batch glob patterns (`--batch`) for mass pre-rendering during curriculum updates.
    - **Pedagogical Comprehension Checks**: Built-in interactive multiple-choice check with immediate validation and explanations.
    - **Schema & Manifest Integration**: Added `narration` definitions in `story.schema.json` and updated `build-manifest.py` with `hasAudio` and `audioDuration`. Both Spanish (`story.a1.01`) and Hungarian mixed-language (`story.a1.unit01`) pilots generated and verified.
