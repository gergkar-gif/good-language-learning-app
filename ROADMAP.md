...
7. **Grammar `teaches` tags that don't match the exercise (added 2026-09-24).** Found while naming every skill (ACHIEVED.md item 89). Some `category: "grammar"` exercises carry a tag for a skill they don't test. Exercises tagged `negation` test the personal *a*, `gerund` tests the passive, `passive-voice-legado` tests *a raíz de*, and `ser-questions` tests *llamarse*. Skill names now follow what the exercises actually test, so learners see the right thing, but the Grammar Driller's topic list and the learner model's per-skill scores are still keyed to the wrong tags. There are also 34 skills, mostly topic-named B1 Latin America ids like `argentina`, `cambiosocial-6` and `consolidation-6`, whose exercises contain no grammar at all. They are comprehension questions and put-the-sentences-in-order tasks, so they're named after the activity ("understanding the text", "putting sentences in order"). To find them, look for skills whose exercises are all `sentence-order` tasks or content-question `multiple-choice` items. Their curated names are "understanding the text" and "putting sentences in order". Fix: retag those exercises (`category: "reading"` or a real grammar skill), then rerun `scripts/build_grammar_index.py`. Its build now warns about any skill missing from `grammar-titles.json`.
