...
Every exercise in `exercises/*/*.json` must carry `category` (`vocabulary | grammar | reading | dialogue | writing | listening` — lesson stages belong in `stage`, not `category`), and every non-`reading` exercise must carry a non-empty `teaches` array of lowercase-hyphenated skill slugs present in `content/hu/indexes/skill-registry.json`.
