...
Every exercise in `exercises/*/*.json` must carry `category` (`vocabulary | grammar | reading | dialogue | writing | listening`), and every non-`reading` exercise must carry a non-empty `teaches` array of lowercase-hyphenated skill slugs present in `content/es-latam/indexes/skill-registry.json`.
